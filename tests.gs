...
function testValidateLabelName() {
  ...
  runTest('Label exceeding 225 chars rejected', function() {
    var longName = '';
    for (var i = 0; i < 230; i++) longName += 'a';
  ...
  runTest('Label at exactly 225 chars accepted', function() {
    var exactName = '';
    for (var i = 0; i < 225; i++) exactName += 'a';
...
function testValidateOrganizationPlan() {
  ...
  runTest('Plan without newLabels rejected', function() {
    ...
    assert(result.valid === false, 'Should be invalid');
    assert(result.errors.some(function(e) { return e.indexOf('newLabels') > -1; }), 'Should mention newLabels');
  });
  ...
  runTest('Migration without from field rejected', function() {
    ...
    assert(result.valid === false, 'Should be invalid');
    assert(result.errors.some(function(e) { return e.indexOf('from') > -1; }), 'Should mention from');
  });
  ...
  runTest('Duplicate migrations generate warning', function() {
    ...
    assert(result.warnings.some(function(w) { return w.indexOf('multiple') > -1; }), 'Should warn about duplicates');
  });
}
